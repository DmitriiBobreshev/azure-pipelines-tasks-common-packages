...
const coverageBaseNameXml = 'cobertura-coverage.xml'
            ...
            if (fs.existsSync(buildPath)) {
                ...
                if (fs.existsSync(path.join(testPath))) {
                    try {
                        ...
                        // util.run(`c8 --all --src ${buildPath} --reports-dir ${testResultsPath} mocha ${mochaOptions} ${testPath}`, true);
                        // util.renameFile(testResultsPath, coverageBaseNameXml, `${child}-coverage.xml`);
